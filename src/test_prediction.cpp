...
int recognize(network<sequential> &nn, const std::string& filename) {
...
int main(int argc, char** argv) {
    if (argc != 4) {
        cout << "USAGE: ./predict ARCH.json WEIGHTS.bin dir"<<endl;
        return 0;
    }

    vector<string> testfiles;
    getdir(argv[3],testfiles);
    ...
    {
        cout<<"testing: "<<testfiles[i]<<endl;
        int outclass=recognize(nn,testfiles[i]);
        res[testfiles[i]]=outclass; //to be used later
        if(outclass==0)
            nbneg++;
        else
            nbpos++;
